...
def initialize_pygame():
    ...
    pygame.display.set_caption("Arkanoid Game")  # Define el título de la ventana
...
def game_mode_menu(window, font, small_font):
    """Muestra el menú para seleccionar entre singleplayer y multiplayer."""
    menu_running = True
    while menu_running:
        ...
        draw_text_centered("Selecciona el modo de juego:", small_font, COLORS['WHITE'], window, WINDOW_WIDTH // 2, GAME_AREA_HEIGHT // 2)
        draw_text_centered("Presiona 1 para Singleplayer", small_font, COLORS['WHITE'], window, WINDOW_WIDTH // 2, GAME_AREA_HEIGHT // 2 + 50)
        draw_text_centered("Presiona 2 para Multiplayer", small_font, COLORS['WHITE'], window, WINDOW_WIDTH // 2, GAME_AREA_HEIGHT // 2 + 100)
...
def difficulty_menu(window, font, small_font):
...
def send_game_state(sock, game_state):
    """Envía el estado del juego al viewer si está abierto."""
    try:
        sock.sendto(json.dumps(game_state).encode(), ('localhost', 12346))  # Enviar al puerto 12346
    except Exception as e:
        print(f"Error enviando datos al viewer: {e}")
...
def main():
    ...
    if connection:
        ...
        database.create_multiplayer_movements_table(connection)  # Crear tabla de movimientos multiplayer
        database.create_multiplayer_games_table(connection)  # Crear tabla de partidas multiplayer
        database.create_voting_tables(connection) # Crear tablas para el sistema de votaciones
    ...
    while running:
        ...
        mode = game_mode_menu(window, font, small_font)

        if mode == 'singleplayer':
            # Crear instancia del juego Arkanoid
            game = ArkanoidGame(window, WINDOW_WIDTH, GAME_AREA_HEIGHT)

            # Selección de dificultad para el modo singleplayer
            difficulty = difficulty_menu(window, font, small_font)
            ...
            # Crear instancia del juego Arkanoid para multiplayer
            game = ArkanoidGame(window, WINDOW_WIDTH, GAME_AREA_HEIGHT)
            ...
            if difficulty in ['Fácil', 'Medio', 'Difícil']:
                ...
                # Guardar el resultado inicial como "En partida" y obtener el game_id
                game_id = database.save_game_result_multiplayer(connection, user_id, difficulty, "En partida")

                game_option = game.game_loop_multiplayer(difficulty, button_positions, font, small_font, connection, user_id, game_id)
                if game_option == 'exit':
                    running = False
            if difficulty == 'exit':
                running = False
