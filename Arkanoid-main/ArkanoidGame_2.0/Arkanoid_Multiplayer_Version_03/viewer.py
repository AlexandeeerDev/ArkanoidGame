...
def receive_game_state(sock):
    """Recibe el estado del juego desde el socket."""
    try:
        sock.settimeout(1.0)  # Evitar que el socket se bloquee indefinidamente
        data, _ = sock.recvfrom(4096)
        if data:
            if data.decode() == 'EXIT':
                return 'EXIT'  # Si se recibe una señal de cierre
            return json.loads(data.decode())  # Si los datos son válidos JSON
...
def draw_waiting_message(window, font):
    ...
    draw_text_centered("Activo únicamente en partida", font, COLORS['WHITE'], window, WINDOW_WIDTH // 2, GAME_AREA_HEIGHT // 2)
