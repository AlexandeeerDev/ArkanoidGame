...
def main():
    ...
    while running:
        ...
        if mode == 'singleplayer':
            ...
            # Definir el número de filas de bloques para el nivel (puedes cambiarlo aquí)
            rows = 4  # Puedes cambiar este valor para ajustar el nivel

            # No se selecciona dificultad en multijugador
            game.create_bricks(rows)
            
            # Iniciar el loop del juego multiplayer
            game_option = game.game_loop_multiplayer(button_positions, font, small_font, connection, user_id)
            if game_option == 'exit':
                running = False
