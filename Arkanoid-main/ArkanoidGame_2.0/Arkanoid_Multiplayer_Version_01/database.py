...
def save_movements_multiplayer(connection):
    """
    ...
    if movements_buffer:
        cursor = connection.cursor()
        cursor.executemany(
            "INSERT INTO multiplayer_movements (user_id, position) VALUES (%s, %s)", 
            movements_buffer
        )
        connection.commit()
        cursor.close()
...
def save_game_result_multiplayer(connection, user_id, result):
    """
    Guarda el resultado final de la partida multijugador.
    """
    cursor = connection.cursor()
    cursor.execute(
        "INSERT INTO multiplayer_games (user_id, result) VALUES (%s, %s)",
        (user_id, result)
    )
    connection.commit()
    cursor.close()
